...
const main = async () => {
  ...
  // 1. Web scraping text normalization
  {
    ...
    const webScrapingRecipe: TransliteratorRecipe = {
      kanjiOldNew: true,
      replaceSpaces: true,
  ...
  // 2. Document standardization
  {
    ...
    const documentRecipe: TransliteratorRecipe = {
      toFullwidth: true,
      replaceSpaces: true,
      kanjiOldNew: true,
      combineDecomposedHiraganasAndKatakanas: true,
  ...
  // 3. Search index preparation
  {
    ...
    const searchRecipe: TransliteratorRecipe = {
      kanjiOldNew: true,
      replaceSpaces: true,
      toHalfwidth: true,
      replaceSuspiciousHyphensToProlongedSoundMarks: true,
  ...
  // 5. Unicode normalization showcase
  {
    ...
    const unicodeRecipe: TransliteratorRecipe = {
      replaceSpaces: true,
    ...
    for (const [text, description] of unicodeSamples) {
      ...
      console.log(`  Result: '${normalized}' (${description})`);
      console.log();
    }
  }
  ...
  // 6. Performance considerations
  {
    ...
    const perfRecipe: TransliteratorRecipe = {
      kanjiOldNew: true,
      replaceSpaces: true,
    };
