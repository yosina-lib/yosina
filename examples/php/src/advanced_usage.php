...
/**
 * Process a list of text samples and display results.
 * @param array<array{string, string}> $samples
 */
function processTextSamples(array $samples): void
{
    echo "Processing text samples with character codes:\n";
    foreach ($samples as [$text, $description]) {
        echo "\n{$description}:\n";
        echo "  Original: '{$text}'\n";
...
function main(): void
{
    ...
    $webScrapingRecipe = new TransliteratorRecipe(
        kanjiOldNew: true,
        replaceSpaces: true,
    ...
    $messyContent = [
        ["これは\u{3000}テスト\u{00a0}です。", "Mixed spaces from different sources"],
    ...
    $documentRecipe = new TransliteratorRecipe(
        toFullwidth: true,
        replaceSpaces: true,
        kanjiOldNew: true,
        combineDecomposedHiraganasAndKatakanas: true,
    ...
    $searchRecipe = new TransliteratorRecipe(
        kanjiOldNew: true,
        replaceSpaces: true,
        toHalfwidth: false,
        replaceSuspiciousHyphensToProlongedSoundMarks: true,
    ...
    $unicodeRecipe = new TransliteratorRecipe(
        replaceSpaces: true,
    ...
    foreach ($unicodeSamples as [$text, $description]) {
        $normalized = $normalizer($text);
        echo "  Result: '{$normalized}' ({$description})\n";
        echo "\n";
    }
    ...
    $perfRecipe = new TransliteratorRecipe(
        kanjiOldNew: true,
        replaceSpaces: true,
    );
