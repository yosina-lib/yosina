...
from .recipes import TransliteratorRecipe, build_transliterator_configs_from_recipe
...
def make_transliterator(
    configs_or_recipe: list[TransliteratorConfig | TransliteratorIdentifier] | TransliteratorRecipe,
) -> Callable[[str], str]:
    ...
    >>> result = transliterator("some japanese text")
    """
    if isinstance(configs_or_recipe, TransliteratorRecipe):
