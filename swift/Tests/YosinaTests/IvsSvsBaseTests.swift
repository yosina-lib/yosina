...
final class IvsSvsBaseTests: XCTestCase {
    func testIvsToBase() {
        ...
        let transliterator = IvsSvsBaseTransliterator(options: options).stringTransliterator
        ...
        XCTAssertEqual(result.count, 1) // Should be just the base character
    ...
    func testCharsetDifference() {
        ...
        let trans90 = IvsSvsBaseTransliterator(options: options90).stringTransliterator

        var options2004 = IvsSvsBaseTransliterator.Options()
        options2004.mode = .ivsOrSvs
        options2004.charset = .unijis2004
        let trans2004 = IvsSvsBaseTransliterator(options: options2004).stringTransliterator
    ...
    func testRoundTrip() {
        ...
        let toIvs = IvsSvsBaseTransliterator(options: optionsToIvs).stringTransliterator

        var optionsToBase = IvsSvsBaseTransliterator.Options()
        optionsToBase.mode = .base
        optionsToBase.charset = .unijis90
        let toBase = IvsSvsBaseTransliterator(options: optionsToBase).stringTransliterator
