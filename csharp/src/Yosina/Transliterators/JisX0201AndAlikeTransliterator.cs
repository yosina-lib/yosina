...
public class JisX0201AndAlikeTransliterator : ITransliterator
{
    ...
    public class Options
    {
        ...
        public bool U005cAsBackslash { get; set; } = true;

        public bool U007eAsFullwidthTilde { get; set; } = true;

        public bool U007eAsWaveDash { get; set; } = true;
    ...
    {
        return new JisX0201CharEnumerator(input.GetEnumerator(), this.options);
    }

    private class JisX0201CharEnumerator : IEnumerable<Character>, IEnumerator<Character>
    {
        ...
        public JisX0201CharEnumerator(IEnumerator<Character> input, Options options)
        {
            this.input = input;
            this.options = options;

            if (this.options.FullwidthToHalfwidth)
            {
                this.forwardMappings = this.BuildForwardMappings();
            }
            else
            {
        ...
        private Dictionary<int, int[]> BuildForwardMappings()
        {
            ...
            if (this.options.ConvertGL)
            {
                ...
                // Handle overrides
                if (this.options.U005cAsYenSign)
                {
                    mappings[0xffe5] = new[] { 0x005c };
                }

                if (this.options.U005cAsBackslash)
                {
                    mappings[0xff3c] = new[] { 0x005c };
                }

                if (this.options.U007eAsFullwidthTilde)
                {
                    mappings[0xff5e] = new[] { 0x007e };
                }

                if (this.options.U007eAsWaveDash)
                {
                    mappings[0x301c] = new[] { 0x007e };
                }

                if (this.options.U007eAsOverline)
                {
                    mappings[0x203e] = new[] { 0x007e };
                }

                if (this.options.U007eAsFullwidthMacron)
                {
                    mappings[0xffe3] = new[] { 0x007e };
                }

                if (this.options.U00a5AsYenSign)
                {
                    mappings[0xffe5] = new[] { 0x00a5 };
                }
            }

            if (this.options.ConvertGR)
            {
            ...
            if (this.options.ConvertUnsafeSpecials)
            {
            ...
            if (this.options.ConvertHiraganas)
            {
