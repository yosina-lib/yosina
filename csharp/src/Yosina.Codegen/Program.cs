    ...
    private static void GenerateKanjiOldNewTransliterator(string dataRoot, string destRoot)
    {
        ...
        foreach (var pair in jsonDoc.RootElement.EnumerateArray())
        {
            if (pair.GetArrayLength() >= 2)
            {
                ...
                {
                    var oldCodepoints = new List<int>();
                    var newCodepoints = new List<int>();

                    foreach (var cp in oldIvs.EnumerateArray())
                    {
                        oldCodepoints.Add(UnicodeUtils.ParseUnicodeCodepoint(cp.GetString() ?? string.Empty));
                    }

                    foreach (var cp in newIvs.EnumerateArray())
                    {
                        newCodepoints.Add(UnicodeUtils.ParseUnicodeCodepoint(cp.GetString() ?? string.Empty));
                    }

                    // Map the base character (first codepoint) from old to new
                    if (oldCodepoints.Count > 0 && newCodepoints.Count > 0)
                    {
                        mappings[oldCodepoints[0]] = newCodepoints[0];
                    }
                }
            }
        }
        ...
        var code = SimpleTransliteratorGenerator.Generate(
            "kanji_old_new",
