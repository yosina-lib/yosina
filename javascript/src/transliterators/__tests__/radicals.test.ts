...
import { default as radicals } from "../radicals.js";

describe("RadicalsTransliterator", () => {
  const transliterator = radicals({});

  const radicalsTestCases = [
  ...
  describe.each(radicalsTestCases)("radical transliteration", (expected, input, description) => {
    test(`${description}`, () => {
  ...
  test("unmapped characters", () => {
    const input = "hello world 123 abc こんにちは 漢字";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(input);
  });

  test("mixed radicals content", () => {
    const input = "部首⺀漢字⼀";
    const expected = "部首冫漢字一";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("kangxi radicals sequence", () => {
    const input = "⼀⼆⼃⼄⼅⼆⼇⼈⼉⼊";
    const expected = "一二丿乙亅二亠人儿入";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("cjk radicals supplement sequence", () => {
    const input = "⺀⺁⺂⺃⺄⺅⺆";
    const expected = "冫厂乛乚乙亻冂";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("hand radical variants", () => {
    const input = "⺘⼿"; // Hand radical variant and Kangxi hand radical
    const expected = "扌手";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("water radical variants", () => {
    const input = "⺡⽔"; // Water radical variant and Kangxi water radical
    const expected = "氵水";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("grass radical variants", () => {
    const input = "⺾⺿⻀⾋"; // Different grass radical variants
    const expected = "艹艹艹艸";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("simplified radicals", () => {
    const input = "⻈⻉⻋⻐⻢⻥⻦"; // Simplified radicals
    const expected = "讠贝车钅马鱼鸟";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });

  test("radicals in context", () => {
    const input = "⼭の⽊を⽔で育てる";
    const expected = "山の木を水で育てる";
    const result = fromChars(transliterator(buildCharArray(input)));
    expect(result).toBe(expected);
  });
});
