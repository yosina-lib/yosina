use yosina::recipes::{
    ...
    ToFullWidthOptions, ToHalfwidthOptions, TransliteratorRecipe,
};
use yosina::transliterators::{Charset, HyphensTransliterationVariant};
use yosina::{make_transliterator, ConfigsOrRecipe};

#[test]
fn test_empty_recipe() {
    let recipe = TransliteratorRecipe::default();
...
fn test_to_fullwidth_basic() {
    ...
    assert_eq!(configs.len(), 1);
    if let yosina::TransliteratorConfig::Jisx0201AndAlikeHalfwidthToFullwidth(opts) = &configs[0] {
        assert!(!opts.u005c_as_yen_sign);
    } else {
...
fn test_to_halfwidth_basic() {
    ...
    if let yosina::TransliteratorConfig::Jisx0201AndAlikeFullwidthToHalfwidth(opts) = &configs[0] {
...
fn test_comprehensive_ordering() {
    ...
    let has_jisx0201 = configs.iter().any(|c| {
        matches!(
            c,
            yosina::TransliteratorConfig::Jisx0201AndAlikeFullwidthToHalfwidth(_)
        )
    });
...
fn test_all_transliterators_enabled() {
    ...
    let jisx_config = configs
        .iter()
        .find_map(|c| match c {
            yosina::TransliteratorConfig::Jisx0201AndAlikeFullwidthToHalfwidth(opts) => Some(opts),
            _ => None,
        })
        .unwrap();
...
fn test_basic_transliteration() {
    ...
    let transliterator = make_transliterator(ConfigsOrRecipe::Recipe(&recipe)).unwrap();
