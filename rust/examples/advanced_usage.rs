...
use yosina::{make_transliterator, ConfigsOrRecipe, TransliteratorConfig, TransliteratorRecipe};
...
fn main() -> Result<()> {
    ...
    let web_scraping_recipe = TransliteratorRecipe {
        kanji_old_new: true,
        replace_spaces: true,
    ...
    let web_normalizer = make_transliterator(ConfigsOrRecipe::Recipe(&web_scraping_recipe))?;
    ...
    let document_recipe = TransliteratorRecipe {
    ...
    let document_standardizer = make_transliterator(ConfigsOrRecipe::Recipe(&document_recipe))?;
    ...
    let search_recipe = TransliteratorRecipe {
        kanji_old_new: true,
        replace_spaces: true,
    ...
    let search_normalizer = make_transliterator(ConfigsOrRecipe::Recipe(&search_recipe))?;
    ...
    let steps = vec![
        ...
        (
            "Width",
            vec![TransliteratorConfig::Jisx0201AndAlikeHalfwidthToFullwidth(
    ...
    for (name, config) in steps {
        let transliterator = make_transliterator(ConfigsOrRecipe::Configs(&config))?;
    ...
    let unicode_recipe = TransliteratorRecipe {
        replace_spaces: true,
    ...
    let unicode_normalizer = make_transliterator(ConfigsOrRecipe::Recipe(&unicode_recipe))?;
    ...
    let perf_recipe = TransliteratorRecipe {
        kanji_old_new: true,
        replace_spaces: true,
        ..Default::default()
    };

    let perf_transliterator = make_transliterator(ConfigsOrRecipe::Recipe(&perf_recipe))?;
