    ...
    fn transliterate<'a, 'b>(
        ...
        for &current_char in chars {
            ...
            if Self::is_prolonged_mark(current_char.c.as_ref()) {
                ...
                if should_process && last_non_prolonged_char.is_some() {
                    let (_last_char, last_char_type) = last_non_prolonged_char.unwrap();
                    ...
                    if (self.prolongables.0 & last_char_type.0) != 0 {
                        ...
                        let nc =
                            pool.new_char_from(Cow::Borrowed(replacement), offset, current_char);
                        offset += replacement.len();
                        result.push(nc);
                        continue;
                    } else {
