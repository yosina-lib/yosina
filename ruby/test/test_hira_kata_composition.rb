...
class TestHiraKataComposition < Minitest::Test
  ...
  def test_non_composable_combinations
    ...
    test_cases = [
      ...
      ['な', '゚'], # N-series + semi-voiced mark (not composable)
  ...
  def test_multiple_character_sequence
    ...
    chars = [
      Yosina::Char.new(c: 'か', offset: 0),
      Yosina::Char.new(c: '゙', offset: 1),  # Should compose: が → が
      Yosina::Char.new(c: 'き', offset: 2),
      Yosina::Char.new(c: '゙', offset: 3),  # Should compose: ぎ → ぎ
      Yosina::Char.new(c: 'あ', offset: 4),
      Yosina::Char.new(c: '゙', offset: 5)   # Should NOT compose: あ゙ (stays separate)
    ]
