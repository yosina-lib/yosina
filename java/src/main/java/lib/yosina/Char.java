...
public class Char {
    ...
    public Char withOffset(int newOffset) {
        return new Char(this.c, newOffset, this.source);
    }

    /**
     * Creates a new character with the same data but different source.
     *
     * @param newSource The new source character for this character
     * @return A new Char instance with the updated source
     */
    public Char withSource(Char newSource) {
        return new Char(this.c, this.offset, newSource);
    }
