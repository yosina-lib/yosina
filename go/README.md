...
func main() {
    ...
    input := "①②③　ⒶⒷⒸ　␀␁␂" // circled numbers, squared letters, ideographic space, control pictures
    result, err := transliterator(input)
    if err != nil {
        log.Fatal(err)
    }
    
    fmt.Println(result) // "hello world" (regular ASCII space)
}
```
...
func main() {
    ...
    chars2 := pool.BuildCharArray("␀␁␂") // control pictures
    result2, err := combinedTransliterator.Transliterate(pool, chars2)
    if err != nil {
        log.Fatal(err)
    }
    
    fmt.Println(yosina.FromChars(result2)) // "NULSOHSTX"
}
```
